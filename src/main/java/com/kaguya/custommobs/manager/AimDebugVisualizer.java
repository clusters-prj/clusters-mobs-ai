 ...
 * <p>
 * 常時トグルにすると、コンソールコマンドの実行結果("Displaying particle...")が
 * 継続的にサーバーログへ流れ込んでしまう(消せない)ため、実行するたびに1回だけ描画する
 * スナップショット方式にしている。見たいタイミングでコマンドを打ち直す運用。
 * <ul>
...
public class AimDebugVisualizer {
    ...
    /** 箱の辺に沿ってパーティクルを打つ間隔(ブロック)。1点ごとにコンソールから{@code /particle}を発行するので粗めにしてある */
    ...
    /**
     * 近くのカスタムMobの当たり判定箱と狙い先を1回だけ描画する。
     *
     * @return 描画した対象の数
     */
